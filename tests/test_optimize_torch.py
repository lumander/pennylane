...
from pennylane.optimize import (RotosolveOptimizerTorch)
...
def bunch():
    class A:
        rotosolvetorch_opt = RotosolveOptimizerTorch()
    return A()
...
class TestOptimizer:
    ...
    @staticmethod
    def rotosolvetorch_step(f, x):
        """Helper function to test the RotosolveTorch optimizers"""
    ...
    def test_rotosolvetorch_optimizer_univar(self, x_start, bunch, tol):
        """Tests that rotosolve optimizer takes one and two steps correctly
        for uni-variate functions."""

        for f in univariate_funcs:
            x_onestep = bunch.rotosolvetorch_opt.step(f, x_start)
            x_onestep_target = self.rotosolvetorch_step(f, x_start)

            assert np.allclose(x_onestep, x_onestep_target, atol=tol, rtol=0)

            x_twosteps = bunch.rotosolvetorch_opt.step(f, x_onestep)
            x_twosteps_target = self.rotosolvetorch_step(f, x_onestep_target)
    ...
    @pytest.mark.parametrize('x_start', [[1.2, 0.2],
                                         ...
                                         [[-0.6], [0.45]],
                                         [[1.3], [-0.9]]])
    def test_rotosolvetorch_optimizer_multivar(self, x_start, bunch, tol):
        """Tests that rotosolve optimizer takes one and two steps correctly
